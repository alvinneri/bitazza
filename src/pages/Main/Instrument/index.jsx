...
const InstrumentContainer = () => {
  ...
  const { instruments, selectedInstrument } = useSelector(
    ...
    >
          ...
          >
            {instruments.map((item) => {
              return (
                ...
                >{`${item.Product1Symbol}/${item.Product2Symbol}`}</MenuItem>
              );
            })}
          </Select>
          <div
      ...
      <div style={{ display: "flex", height: "700px", padding: "1em" }}>
        ...
        <div style={{ padding: "1em" }}>
          <label className={classes.digits}>
            {" "}
            {getChange(selectedInstrument[0]?.InstrumentId)}
          </label>
          <p className={classes.label}>CHANGE</p>
          <label className={classes.digits}>
            {selectedInstrument[0]?.MinimumPrice}
          </label>
          <p className={classes.label}>MINIMUM PRICE</p>
          <label className={classes.digits}>
            {selectedInstrument[0]?.MinimumQuantity}
          </label>
          <p className={classes.label}>MINIMUM QUANTITY</p>
          <label className={classes.digits}>
            {selectedInstrument[0]?.PriceIncrement}
          </label>
          <p className={classes.label}>PRICE INCREMENT</p>
          <label className={classes.digits}>
            {selectedInstrument[0]?.QuantityIncrement}
          </label>
